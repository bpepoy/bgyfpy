...
# ===========================================================================
# Data generation — analytics.json
# ===========================================================================
...
def build_analytics(
    ...
    try:
        ...
        if existing.get("_built_at") and not force_clean:
            return {"status": "already_built", "built_at": existing["_built_at"],
                    "note": "Use force_clean=true to rebuild"}

        # ── load files ───────────────────────────────────────────────────────
        results_raw  = _load_json(_get_data_path("results.json"))
        matchups_raw = _load_json(_get_data_path("matchups.json"))
        rosters_raw  = _load_json(_get_data_path("rosters.json"))
        stats_raw    = _load_json(_get_data_path("player_stats.json"))
        info_raw     = _load_json(_get_data_path("player_info.json"))
        rules_raw    = _load_json(_get_data_path("rules.json"))
        ices_raw     = _load_json(_get_data_path("ices.json"))
        drafts_raw   = _load_json(_get_data_path("drafts.json"))
        tx_raw       = _load_json(_get_data_path("transactions.json"))
        payouts_raw  = _load_json(_get_data_path("payouts.json"))

        # ── restrict to known active + historical members ─────────────────────
        KNOWN_MEMBERS = {
            "blake","brian","frank","jake","joey",
            "jordan","kyle","nick","rob","sam","zef"
        }

        finished = {yr: s for yr, s in results_raw.items()
                    if str(yr).isdigit() and s.get("is_finished")}
        all_yrs  = sorted(finished.keys(), key=int)

        def _dn(mid):
            for yr in sorted(finished.keys(), reverse=True):
                m = finished[yr].get("managers", {}).get(mid, {})
                if m.get("display_name"): return m["display_name"]
            return mid.title()

        # ── FAAB / Auction / stats constants ─────────────────────────────────
        FAAB_START    = 2015
        AUCTION_START = 2023
        REAL_ICE_START= 2023
        STATS_START   = 2015
        PTS_BREAKDOWN_START = 2023

        # stat_id bucketing for points breakdown
        YARDS_IDS      = {"1","2","9","11","12"}
        TD_IDS         = {"5","10","13","15","57","49","35"}
        TWO_PT_SF_IDS  = {"16","36"}

        # ── per-season computation ────────────────────────────────────────────
        # We compute everything season-by-season first, then aggregate
        season_data: dict = {}   # yr → full season analytics

        # Outer accumulators (all-time)
        actual_career:  dict = {}   # mid → {w,l,t,games}
        theo_career:    dict = {}   # mid → {w,l,t}
        bb_career:      dict = {}   # mid → {w,l,t}

        all_weekly_pf:  list = []
        all_season_pf:  list = []

        faab_bids:      list = []
        faab_season_rem:list = []
        faab_rem_map:   dict = {}   # mid → [remaining values]

        auction_bids:   list = []
        draft_top_picks:dict = {}   # player_name → {count, years, positions}

        pos_week_top:   dict = {"QB":[],"WR":[],"RB":[],"TE":[]}
        pos_season_top: dict = {"QB":[],"WR":[],"RB":[],"TE":[]}

        mgr_player_wks: dict = {}   # (mid, nm) → weeks
        pair_counts:    dict = {}
        traded_counts:  dict = {}
        play_twice:     dict = {}

        pos_rank_acc:   dict = {}   # mid → {pos → [weekly pts]}

        td_for:         dict = {}
        td_against:     dict = {}
        pts_breakdown:  dict = {}   # mid → {yr → {yards,tds,2pt,other}} + totals

        ice_by_mgr_real:  dict = {}
        ice_by_mgr_theo:  dict = {}
        ice_by_pos_real:  dict = {}
        ice_by_pos_theo:  dict = {}
        ice_vs_real:      dict = {}   # opp → {mid: count} real ices you face
        ice_vs_theo:      dict = {}

        champ_players: dict = {}

        cumulative_wins: dict = {}
        bar_chart_race:  list = []

        # ── best-ball lineup helper ───────────────────────────────────────────
        def _best_ball_score(mid, yr, wk_key, wk_stats, yr_roster, yr_info, yr_rules):
            wk_team  = yr_roster.get(wk_key, {}).get(mid, {})
            slots    = wk_team.get("players", [])
            if not slots: return None

            slots_def = yr_rules.get("roster_positions", [])
            slot_reqs: dict = {}
            for slot in slots_def:
                pos = slot if isinstance(slot, str) else slot.get("position","")
                cnt = 1 if isinstance(slot, str) else (slot.get("count") or 1)
                if pos and pos not in ("BN","IR","IR+",""):
                    slot_reqs[pos] = slot_reqs.get(pos, 0) + cnt

            flex_count = slot_reqs.pop("W/R/T", 0) + slot_reqs.pop("FLEX", 0)
            flex_pos   = {"WR","RB","TE"}
            ...
            for slot in slots:
                ...
                pk  = slot.get("player_key") or ""
                pi  = yr_info.get(pk, {})
                pos_raw = pi.get("position") or slot.get("selected_position") or ""
                pos = pos_raw.split("/")[0].strip() if "/" in pos_raw else pos_raw
                pd  = wk_stats.get(pk)
                pts = float(pd.get("fantasy_points") or 0) if isinstance(pd, dict) else 0.0
                players.append({"pos": pos, "pts": pts})
        ...
        for yr in all_yrs:
            ...
            yr_roster = rosters_raw.get(yr, {})
            yr_stats  = stats_raw.get(yr, {})
            yr_info   = (info_raw.get(yr, {}) or {}).get("players", {})
            yr_rules  = rules_raw.get(yr, {})
            ps        = yr_mu.get("playoff_start") or 99
            yr_mgrs   = {mid: m for mid, m in finished[yr].get("managers", {}).items()
                         if mid in KNOWN_MEMBERS}

            # Per-season W-L accumulators
            yr_actual:  dict = {mid: {"w":0,"l":0,"t":0,"pf":0.0,"pa":0.0} for mid in yr_mgrs}
            yr_theo:    dict = {mid: {"w":0.0,"l":0.0,"t":0.0} for mid in yr_mgrs}
            yr_bb:      dict = {mid: {"w":0.0,"l":0.0,"t":0.0,"pf":0.0} for mid in yr_mgrs}

            # owner map (last week)
            owner_map: dict = {}
            wk_keys_all = sorted([k for k in yr_roster if k.startswith("week_")],
                                   key=lambda x: int(x.split("_")[1]))
            if wk_keys_all:
                lw = yr_roster.get(wk_keys_all[-1], {})
                for mid, team in lw.items():
                    if not isinstance(team, dict): continue
                    for slot in team.get("players", []):
                        pk = slot.get("player_key") if isinstance(slot, dict) else None
                        if pk: owner_map[pk] = {"manager_id": mid, "display_name": _dn(mid)}
            ...
            for wk_entry in yr_mu.get("weeks", []):
                ...
                for m in wk_entry.get("matchups", []):
                    ...
                    if len(teams) == 2:
                        wk_opp_map[(teams[0], wk_num)] = teams[1]
                        wk_opp_map[(teams[1], wk_num)] = teams[0]
            ...
            for wk_entry in yr_mu.get("weeks", []):
                ...
                for m in wk_entry.get("matchups", []):
                    ...
                    for t in m.get("teams", []):
                        ...
                        wk_scores[mid] = pts
                        if yr_int >= STATS_START and wk_stats:
                            opt = _best_ball_score(mid, yr, wk_key, wk_stats,
                                                   yr_roster, yr_info, yr_rules)
                            wk_bb[mid] = opt if opt is not None else pts

                        # Actual W-L
                        if mid in yr_actual:
                            if t.get("is_winner"):     yr_actual[mid]["w"] += 1
                            elif m.get("is_tied"):      yr_actual[mid]["t"] += 1
                            else:                       yr_actual[mid]["l"] += 1
                            yr_actual[mid]["pf"] = round(yr_actual[mid]["pf"] + pts, 2)

                all_scores = list(wk_scores.values())
                if len(all_scores) < 2: continue
                n = len(all_scores)
                ...
                for mid, my_pts in wk_scores.items():
                    ...
                    # Theoretical
                    yr_theo[mid]["w"] += round(sum(1 for s in opps if my_pts > s) / d, 4)
                    yr_theo[mid]["l"] += round(sum(1 for s in opps if my_pts < s) / d, 4)
                    yr_theo[mid]["t"] += round(sum(1 for s in opps if my_pts == s) / d, 4)

                    # Best ball
                    if mid in wk_bb:
                        bb_pts = wk_bb[mid]
                        yr_bb[mid]["pf"] = round(yr_bb[mid]["pf"] + bb_pts, 2)
                        yr_bb[mid]["w"] += round(sum(1 for s in opps if bb_pts > s) / d, 4)
                        yr_bb[mid]["l"] += round(sum(1 for s in opps if bb_pts < s) / d, 4)
                        yr_bb[mid]["t"] += round(sum(1 for s in opps if bb_pts == s) / d, 4)

                # Weekly PF
                for mid, pts in wk_scores.items():
                    if pts <= 0: continue
                    all_weekly_pf.append({"manager_id":mid,"display_name":_dn(mid),
                                          "year":yr_int,"week":wk_num,"points":round(pts,2)})
                    yr_actual[mid]["pa"] = round(yr_actual[mid].get("pa",0.0) +
                                                  (wk_scores.get(wk_opp_map.get((mid,wk_num),""),0)),2)
                ...
                for m in wk_entry.get("matchups", []):
                    ...
                    teams = [t.get("manager_id") for t in m.get("teams",[])
                             if t.get("manager_id") in KNOWN_MEMBERS]
                    if len(teams)==2:
                        for mid,opp in [(teams[0],teams[1]),(teams[1],teams[0])]:
                            play_twice.setdefault(mid,{}).setdefault(opp,{"seasons":0,"twice":0})
                            play_twice[mid][opp]["seasons"] += 1

            # Count twice plays per season
            for yr2, yr_mu2 in {yr: yr_mu}.items():
                ps2 = yr_mu2.get("playoff_start") or 99
                game_counts: dict = {}
                for wk_entry in yr_mu2.get("weeks",[]):
                    if wk_entry.get("week",0) >= ps2: continue
                    for m in wk_entry.get("matchups",[]):
                        if m.get("is_consolation"): continue
                        teams = [t.get("manager_id") for t in m.get("teams",[])
                                 if t.get("manager_id") in KNOWN_MEMBERS]
                ...
                for (m1,m2), cnt in game_counts.items():
                    for mid,opp in [(m1,m2),(m2,m1)]:
                        if cnt >= 2:
                            play_twice.setdefault(mid,{}).setdefault(opp,{"seasons":0,"twice":0})
                            play_twice[mid][opp]["twice"] += 1

            # Season PF
            for mid, acc in yr_actual.items():
                rs = yr_mgrs[mid].get("regular_season", {})
                g  = (rs.get("wins") or 0)+(rs.get("losses") or 0)+(rs.get("ties") or 0)
                all_season_pf.append({"manager_id":mid,"display_name":_dn(mid),
                                      "year":yr_int,"total_pf":round(acc["pf"],2),
                                      "avg_pf":round(acc["pf"]/g,2) if g else None})

            # Standings comparison
            actual_seeds = {mid: yr_mgrs[mid].get("regular_season",{}).get("rank")
                            for mid in yr_mgrs}
            actual_po    = {mid for mid,rk in actual_seeds.items() if rk and rk <= 4}

            theo_sorted  = sorted(yr_theo.items(), key=lambda x: -x[1]["w"])
            theo_po      = {mid for mid,_ in theo_sorted[:4]}
            bb_sorted    = sorted(yr_bb.items(), key=lambda x: -x[1]["w"])
            bb_po        = {mid for mid,_ in bb_sorted[:4]}

            theo_changed = actual_po != theo_po
            bb_changed   = actual_po != bb_po

            yr_games = {mid: (yr_actual[mid]["w"]+yr_actual[mid]["l"]+yr_actual[mid]["t"])
                        for mid in yr_actual}

            season_data[yr] = {
                "year": yr_int,
                "standings": {},
                "theoretical_changed": theo_changed,
                "actual_playoff":   sorted(actual_po),
                "theo_playoff":     sorted(theo_po),
                "bb_playoff":       sorted(bb_po),
                "theo_gained":      sorted(theo_po - actual_po),
                "theo_lost":        sorted(actual_po - theo_po),
                "bb_gained":        sorted(bb_po - actual_po),
                "bb_lost":          sorted(actual_po - bb_po),
                "managers": {},
            }

            for mid in yr_mgrs:
                rs  = yr_mgrs[mid].get("regular_season", {})
                g   = yr_games.get(mid, 1) or 1
                th  = yr_theo.get(mid, {})
                b   = yr_bb.get(mid,   {})
                act = yr_actual.get(mid, {})
                luck= round(act.get("w",0) - th.get("w",0), 2)

                season_data[yr]["managers"][mid] = {
                    "display_name":  _dn(mid),
                    "actual":  {"w": act["w"],"l":act["l"],"t":act["t"],"pf":round(act["pf"],2)},
                    "theoretical": {"w":round(th["w"],1),"l":round(th["l"],1),"t":round(th["t"],1)},
                    "best_ball":   {"w":round(b.get("w",0),1),"l":round(b.get("l",0),1),
                                    "t":round(b.get("t",0),1),"pf":round(b.get("pf",0),2)},
                    "luck_factor": luck,
                    "rs_rank":     rs.get("rank"),
                    "theo_rank":   next((i+1 for i,(m,_) in enumerate(theo_sorted) if m==mid), None),
                    "bb_rank":     next((i+1 for i,(m,_) in enumerate(bb_sorted)   if m==mid), None),
                }

                # Accumulate to career
                for d, src in [(actual_career, act),(theo_career, th),(bb_career, b)]:
                    if mid not in d: d[mid] = {"w":0.0,"l":0.0,"t":0.0,"seasons":0,"games":0}
                    d[mid]["w"] += src.get("w",0); d[mid]["l"] += src.get("l",0)
                    d[mid]["t"] += src.get("t",0)
                d = actual_career[mid]; d["seasons"] += 1; d["games"] += g

            # player stats this season
            if yr_stats:
                yr_season_pts: dict = {}   # for pos_season_top
                for wk_key2, wk_data2 in yr_stats.items():
                    if not isinstance(wk_data2, dict): continue
                    wk_num2 = int(wk_key2.split("_")[1])
                    if wk_num2 >= ps: continue
                    wk_roster2 = yr_roster.get(wk_key2, {})
                    for pk, pd2 in wk_data2.items():
                        if not isinstance(pd2, dict): continue
                        try: fp2 = float(pd2.get("fantasy_points") or 0)
                        except: continue
                        if fp2 <= 0: continue
                        pi2 = yr_info.get(pk, {})
                        pos_raw2 = pi2.get("position") or ""
                        pos2 = pos_raw2.split("/")[0].strip() if "/" in pos_raw2 else pos_raw2
                        nm2  = pi2.get("name") or pk
                        owner2 = owner_map.get(pk, {})
                        mid2   = owner2.get("manager_id") or ""

                        # pos_week_top
                        if pos2 in pos_week_top:
                            pos_week_top[pos2].append({
                                "player_name":nm2,"position":pos2,
                                "manager_id":mid2,"display_name":owner2.get("display_name"),
                                "year":yr_int,"week":wk_num2,"points":round(fp2,2),
                            })

                        # season pts accumulator
                        yr_season_pts.setdefault(pk, {"nm":nm2,"pos":pos2,"pts":0.0,"owner":owner2})
                        yr_season_pts[pk]["pts"] += fp2

                        # mgr+player weeks (roster)
                        if mid2 and mid2 in KNOWN_MEMBERS:
                            key3 = (mid2, nm2)
                            mgr_player_wks[key3] = mgr_player_wks.get(key3,0) + 1

                        # pos_rank_acc
                        wk_roster_slot = yr_roster.get(wk_key2, {})
                        my_team_slot = wk_roster_slot.get(mid2, {})
                        is_starting = any(
                            s.get("player_key") == pk and s.get("is_starting")
                            for s in my_team_slot.get("players",[])
                            if isinstance(s, dict)
                        )
                        if is_starting and mid2 in KNOWN_MEMBERS:
                            pos_rank_acc.setdefault(mid2, {}).setdefault(pos2, []).append(fp2)

                        # points breakdown (2023+)
                        if yr_int >= PTS_BREAKDOWN_START and mid2 in KNOWN_MEMBERS:
                            stats_detail = pd2.get("stats") or {}
                            yards = tds = two_pt = other = 0.0
                            yr_rules_cats = yr_rules.get("stat_categories", [])
                            ppu_map = {str(sc.get("stat_id","")): sc.get("points_per_unit") or 0
                                       for sc in yr_rules_cats}
                            for sid, val in stats_detail.items():
                                sid_str = str(sid)
                                try: pts_contrib = float(val) * float(ppu_map.get(sid_str, 0))
                                except: continue
                                if sid_str in YARDS_IDS:     yards  += pts_contrib
                                elif sid_str in TD_IDS:      tds    += pts_contrib
                                elif sid_str in TWO_PT_SF_IDS: two_pt += pts_contrib
                                else:                          other  += pts_contrib
                            entry_bd = pts_breakdown.setdefault(mid2, {}).setdefault(str(yr_int), {
                                "yards":0.0,"touchdowns":0.0,"two_pt_safety":0.0,"other":0.0
                            })
                            entry_bd["yards"]         = round(entry_bd["yards"]         + yards,  2)
                            entry_bd["touchdowns"]    = round(entry_bd["touchdowns"]    + tds,    2)
                            entry_bd["two_pt_safety"] = round(entry_bd["two_pt_safety"] + two_pt, 2)
                            entry_bd["other"]         = round(entry_bd["other"]         + other,  2)

                        # TD for/against (2022+)
                        if yr_int >= 2022 and is_starting:
                            stats_d = pd2.get("stats") or {}
                            wk_tds  = 0
                            for sid, val in stats_d.items():
                                if str(sid) in TD_IDS:
                                    try: wk_tds += int(float(val))
                                    except: pass
                            if mid2: td_for[mid2] = td_for.get(mid2,0) + wk_tds
                            opp3 = wk_opp_map.get((mid2, wk_num2))
                            if opp3: td_against[opp3] = td_against.get(opp3,0) + wk_tds

                for pk, info in yr_season_pts.items():
                    pos3 = info["pos"]
                    if pos3 in pos_season_top:
                        pos_season_top[pos3].append({
                            "player_name":info["nm"],"position":pos3,
                            "manager_id":info["owner"].get("manager_id"),
                            "display_name":info["owner"].get("display_name"),
                            "year":yr_int,"season_pts":round(info["pts"],2),
                        })

            # Championship roster
            champ_mid = next((mid for mid, m in yr_mgrs.items()
                              if (m.get("playoffs") or m.get("playoff",{})).get("finish")==1), None)
            if champ_mid:
                wk_keys_r = sorted([k for k in yr_roster if k.startswith("week_")],
                                    key=lambda x: int(x.split("_")[1]))
                if wk_keys_r:
                    slots_c = yr_roster.get(wk_keys_r[-1],{}).get(champ_mid,{}).get("players",[])
                    for slot in slots_c:
                        if not isinstance(slot, dict): continue
                        pk = slot.get("player_key") or ""
                        pi = yr_info.get(pk, {})
                        ...
                        if not nm: continue
                        weight = 5 if slot.get("is_starting") else (1 if slot.get("is_on_ir") else 3)
                        champ_players.setdefault(nm, {"weight":0,"appearances":0,
                                                       "position":pi.get("position"),"years":[]})
                        champ_players[nm]["weight"]      += weight
                        champ_players[nm]["appearances"] += 1
                        champ_players[nm]["years"].append(yr_int)

            # Bar chart race
            for mid in KNOWN_MEMBERS:
                cumulative_wins[mid] = cumulative_wins.get(mid,0)
            for mid, m in yr_mgrs.items():
                if mid in KNOWN_MEMBERS:
                    cumulative_wins[mid] += m.get("regular_season",{}).get("wins") or 0
            bar_chart_race.append({"year":yr_int,
                                   "totals":{mid:cumulative_wins.get(mid,0) for mid in KNOWN_MEMBERS}})
            ...
            if yr_int >= FAAB_START:
                ...
                for move in yr_tx.get("moves",[]):
                    mid_f = move.get("manager_id") or move.get("manager") or ""
                    if mid_f not in KNOWN_MEMBERS: continue
                    for added in move.get("added",[]):
                        bid = added.get("waiver_bid")
                        if bid:
                            try:
                                bid_int = int(bid)
                                spent_this_yr[mid_f] = spent_this_yr.get(mid_f,0)+bid_int
                                faab_bids.append({"manager_id":mid_f,"display_name":_dn(mid_f),
                                                  "player_name":added.get("name"),
                                                  "position":added.get("position"),
                                                  "bid":bid_int,"year":yr_int})
                            except: pass
                for mid_f, spent in spent_this_yr.items():
                    rem = 200 - spent
                    faab_season_rem.append({"manager_id":mid_f,"display_name":_dn(mid_f),
                                            "year":yr_int,"spent":spent,"remaining":rem})
                    faab_rem_map.setdefault(mid_f,[]).append(rem)

            # Auction
            yr_draft = drafts_raw.get(yr, {})
            if yr_int >= AUCTION_START and yr_draft.get("draft_type")=="auction":
                for p in yr_draft.get("picks",[]):
                    cost = p.get("cost")
                    mid_a= p.get("manager_id") or ""
                    if not cost or mid_a not in KNOWN_MEMBERS: continue
                    try:
                        auction_bids.append({"manager_id":mid_a,"display_name":_dn(mid_a),
                                             "player_name":p.get("player_name"),
                                             "position":p.get("position"),
                                             "cost":int(cost),"year":yr_int})
                    except: pass

            # Top draft picks (snake round 1 OR auction top 10 cost)
            draft_type_yr = yr_draft.get("draft_type","snake")
            picks_yr      = yr_draft.get("picks",[])
            if draft_type_yr == "snake":
                r1_picks = [p for p in picks_yr if (p.get("round") or 0)==1
                            and (p.get("manager_id") or "") in KNOWN_MEMBERS]
                for p in r1_picks:
                    nm_p = (p.get("player_name") or "").strip()
                    if not nm_p: continue
                    draft_top_picks.setdefault(nm_p,{"count":0,"years":[],"positions":set()})
                    draft_top_picks[nm_p]["count"] += 1
                    draft_top_picks[nm_p]["years"].append(yr_int)
                    draft_top_picks[nm_p]["positions"].add(p.get("position") or "")
            else:
                # top 10 by cost
                valid = [p for p in picks_yr if p.get("cost") and
                         (p.get("manager_id") or "") in KNOWN_MEMBERS]
            ...
            for trade in yr_tx.get("trades",[]):
                ...
                for side in ["a_received","b_received","trader_receives","tradee_receives"]:
                    for p in trade.get(side,[]):
                        nm_t = (p.get("name") or p.get("player_name") or "").strip()
                        pos_t= p.get("position") or ""
                        if nm_t and pos_t not in ("DEF","D/ST","K"):
                            traded_counts[nm_t] = traded_counts.get(nm_t,0)+1

            # Ices
            yr_ices = ices_raw.get(yr, [])
            if isinstance(yr_ices, list):
                for ice in yr_ices:
                    if ice.get("is_playoffs"): continue
                    mid_i = ice.get("manager_id") or ""
                    pos_i = ice.get("position")   or ""
                    if mid_i not in KNOWN_MEMBERS: continue
                    is_real = yr_int >= REAL_ICE_START
                    # by manager
                    if is_real:
                        ice_by_mgr_real[mid_i] = ice_by_mgr_real.get(mid_i,0)+1
                    ice_by_mgr_theo[mid_i] = ice_by_mgr_theo.get(mid_i,0)+1
                    # by position
                    if pos_i:
                        if is_real:
        ...
        for mid in sorted(KNOWN_MEMBERS):
            act = actual_career.get(mid, {})
            th  = theo_career.get(mid,  {})
            b   = bb_career.get(mid,    {})
            seasons = act.get("seasons") or 1
            games   = act.get("games")   or 1
            aw = act.get("w",0); al=act.get("l",0); at=act.get("t",0)
            tw = th.get("w",0);  tl=th.get("l",0);  tt=th.get("t",0)
            bw = b.get("w",0);   bl=b.get("l",0);   bt=b.get("t",0)
            luck = round(aw - tw, 2)
            wl_records.append({
                "manager_id":   mid,
                "display_name": _dn(mid),
                "actual": {
                    "total":   {"w":aw,"l":al,"t":at,
                                "win_pct":round(aw/games,4) if games else None},
                    "per_season":{"w":round(aw/seasons,1),"l":round(al/seasons,1),
                                  "t":round(at/seasons,1)},
                },
                "theoretical": {
                    "total":   {"w":round(tw,1),"l":round(tl,1),"t":round(tt,1),
                                "win_pct":round(tw/games,4) if games else None},
                    "per_season":{"w":round(tw/seasons,1),"l":round(tl/seasons,1),
                                  "t":round(tt/seasons,1)},
                },
                "best_ball": {
                    "total":   {"w":round(bw,1),"l":round(bl,1),"t":round(bt,1),
                                "win_pct":round(bw/games,4) if games else None},
                    "per_season":{"w":round(bw/seasons,1),"l":round(bl/seasons,1),
                                  "t":round(bt/seasons,1)},
                },
                "luck_factor":  luck,
                "luck_label":   "lucky" if luck>0 else ("unlucky" if luck<0 else "neutral"),
                "seasons":      seasons,
            })
        wl_records.sort(key=lambda x: -(x["actual"]["total"].get("win_pct") or 0))

        # Sort pos records
        for pos in pos_week_top:
            pos_week_top[pos]   = sorted(pos_week_top[pos],   key=lambda x: -x["points"])[:10]
            pos_season_top[pos] = sorted(pos_season_top[pos], key=lambda x: -x["season_pts"])[:10]

        all_weekly_pf.sort(key=lambda x: -x["points"])
        all_season_pf.sort(key=lambda x: -x["total_pf"])

        # Draft top picks sorted by count
        top_draft_picks_list = sorted(
            [{"player_name":nm,"count":v["count"],
              "years":sorted(v["years"]),
              "positions":sorted(v["positions"])}
             for nm, v in draft_top_picks.items() if v["count"] >= 2],
            key=lambda x: -x["count"]
        )[:10]

        # Points breakdown totals per manager
        pts_breakdown_out: dict = {}
        for mid, yr_breakdown in pts_breakdown.items():
            totals = {"yards":0.0,"touchdowns":0.0,"two_pt_safety":0.0,"other":0.0}
            by_year = {}
            for yr_str, cats in sorted(yr_breakdown.items()):
                by_year[yr_str] = cats
                for cat, val in cats.items():
                    totals[cat] = round(totals[cat] + val, 2)
            pts_breakdown_out[mid] = {
                "display_name": _dn(mid),
                "totals":       totals,
                "by_year":      by_year,
            }
        ...
        for pos in ["QB","WR","RB","TE","K","DEF"]:
            mgr_avgs = []
            for mid, pos_data in pos_rank_acc.items():
                if mid not in KNOWN_MEMBERS: continue
                pts = pos_data.get(pos, [])
                if pts:
                    mgr_avgs.append({"manager_id":mid,"display_name":_dn(mid),
                                     "avg_pts":round(sum(pts)/len(pts),2),"weeks":len(pts)})
            mgr_avgs.sort(key=lambda x: -x["avg_pts"])
            for i, e in enumerate(mgr_avgs): e["rank"] = i+1
            pos_rankings[pos] = mgr_avgs

        # Touchdowns
        touchdown_records = sorted([
            {"manager_id":mid,"display_name":_dn(mid),
             "td_for":td_for.get(mid,0),"td_against":td_against.get(mid,0),
             "td_diff":td_for.get(mid,0)-td_against.get(mid,0)}
            for mid in KNOWN_MEMBERS
        ], key=lambda x: -x["td_for"])

        # FAAB aggregates
        faab_bids.sort(key=lambda x: -x["bid"])
        faab_season_rem.sort(key=lambda x: -x["remaining"])
        avg_faab_rem = [{"manager_id":m,"display_name":_dn(m),
                          "avg_remaining":round(sum(v)/len(v),1)}
                         for m,v in sorted(faab_rem_map.items(),
                                           key=lambda x: -sum(x[1])/len(x[1]))]

        # Best season per manager
        best_seasons_out = []
        for mid in KNOWN_MEMBERS:
            best = None
            for yr, season in finished.items():
                m = season.get("managers",{}).get(mid)
                if not m: continue
                rs  = m.get("regular_season",{})
                wp  = rs.get("win_pct") or 0
                if best is None or wp > best["win_pct"]:
                    best = {"manager_id":mid,"display_name":_dn(mid),
                            "year":int(yr),"wins":rs.get("wins"),"losses":rs.get("losses"),
                            "win_pct":round(wp,4),"avg_pf":round(rs.get("avg_points_for") or 0,2),
                            "rank":rs.get("rank")}
            if best: best_seasons_out.append(best)

        # Double play
        twice_summary: list = []
        for mid in KNOWN_MEMBERS:
            opps_d = play_twice.get(mid,{})
            opp_list = []
            for opp, data in opps_d.items():
                s   = max(data["seasons"]//2, 1)  # divided by 2 since we added per-matchup
                tw2 = data["twice"]//2
                exp = round(s * 0.6667,2)
                apc = round(tw2/s,4) if s else 0
                opp_list.append({"opponent_id":opp,"display_name":_dn(opp),
                                  "seasons":s,"twice_count":tw2,"expected":exp,
                                  "actual_pct":apc,"deviation":round(apc-0.6667,4)})
            opp_list.sort(key=lambda x: abs(x["deviation"]), reverse=True)
            twice_summary.append({"manager_id":mid,"display_name":_dn(mid),
                                   "furthest_from_expected":opp_list[0] if opp_list else None,
                                   "all_opponents":opp_list})

        # Trade records
        top_trade_partners = sorted(
            [{"manager_1":{"manager_id":p[0],"display_name":_dn(p[0])},
              "manager_2":{"manager_id":p[1],"display_name":_dn(p[1])},
              "total_trades":cnt}
             for p,cnt in pair_counts.items()],
            key=lambda x: -x["total_trades"])[:10]

        top_traded_players = sorted(
            [{"player_name":nm,"times_in_trades":cnt}
             for nm,cnt in traded_counts.items() if cnt>=2],
            key=lambda x: -x["times_in_trades"])[:10]

        # Champ players (cap 10, min 2 appearances)
        champ_top10 = sorted(
            [(nm,v) for nm,v in champ_players.items() if v["appearances"]>=2],
            key=lambda x: -x[1]["weight"])[:10]
        championship_players_out = [
            {"player_name":nm,"position":v["position"],"appearances":v["appearances"],
             "weighted_score":v["weight"],"years":sorted(v["years"])}
            for nm,v in champ_top10
        ]

        # Ice records
        def _fmt_ice_mgr(d):
            return sorted([{"manager_id":m,"display_name":_dn(m),"count":c}
                            for m,c in d.items()], key=lambda x: -x["count"])
        def _fmt_ice_pos(d):
            return {pos: _fmt_ice_mgr(counts) for pos,counts in d.items()}

        ice_records = {
            "real": {
                "note":        f"Regular season only, {REAL_ICE_START}+",
                "by_manager":  _fmt_ice_mgr(ice_by_mgr_real),
                "by_position": _fmt_ice_pos(ice_by_pos_real),
                "opponent_ices_against_you": {
                    m: _fmt_ice_mgr(opps) for m,opps in ice_vs_real.items()
                },
            },
            "theoretical": {
                "note":        "All regular season years retroactively applied",
                "by_manager":  _fmt_ice_mgr(ice_by_mgr_theo),
                "by_position": _fmt_ice_pos(ice_by_pos_theo),
                "opponent_ices_against_you": {
                    m: _fmt_ice_mgr(opps) for m,opps in ice_vs_theo.items()
                },
            },
        }
        ...
        output = {
            ...
            "_seasons_covered":   len(all_yrs),
            "_first_season":      int(all_yrs[0])  if all_yrs else None,
            "_last_season":       int(all_yrs[-1]) if all_yrs else None,
            "_members":           sorted(KNOWN_MEMBERS),
            "wl_records":         wl_records,
            "best_season_per_manager": best_seasons_out,
            "season_data":        season_data,
            "top10_weekly_pf":    all_weekly_pf[:10],
            "bottom10_weekly_pf": list(reversed(all_weekly_pf[-10:])),
            "top10_season_pf":    all_season_pf[:10],
            "bottom10_season_pf": list(reversed(all_season_pf[-10:])),
            "bar_chart_race":     bar_chart_race,
            "championship_players": championship_players_out,
            "ice_records":        ice_records,
            "faab_records": {
                "top10_bids":         faab_bids[:10],
                "bottom10_remaining": faab_season_rem[:10],
                "avg_remaining":      avg_faab_rem,
            },
            "auction_records": {
                "top10_bids": sorted(auction_bids, key=lambda x: -x["cost"])[:10],
            },
            "top_draft_picks":         top_draft_picks_list,
            "position_week_records":   pos_week_top,
            "position_season_records": pos_season_top,
            "top10_mgr_player_weeks":  sorted(
                [{"manager_id":mid,"display_name":_dn(mid),"player_name":nm,
                  "weeks_together":wks,"approx_seasons":round(wks/16,1)}
                 for (mid,nm),wks in mgr_player_wks.items()
                 if mid in KNOWN_MEMBERS],
                key=lambda x: -x["weeks_together"])[:10],
            "trade_records": {
                "top_partners":        top_trade_partners,
                "most_traded_players": top_traded_players,
            },
            "double_play_frequency":   twice_summary,
            "position_rankings":       pos_rankings,
            "points_breakdown":        pts_breakdown_out,
            "touchdown_records":       touchdown_records,
        }
...
def analytics_status():
    """Check if analytics.json has been built and when."""
    try:
        data = _load_json(_get_data_path("analytics.json"))
        if not data:
            return {"status": "not_built", "note": "Run /league/data/analytics/build-all"}
        return {
            "status":    "built",
            "built_at":  data.get("_built_at"),
            "seasons":   data.get("_seasons_covered"),
            "first_season": data.get("_first_season"),
            "last_season":  data.get("_last_season"),
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/data/analytics/download")
def download_analytics():
    """Returns analytics.json for local save."""
    try:
        data = _load_json(_get_data_path("analytics.json"))
        if not data:
            raise HTTPException(status_code=404,
                detail="analytics.json not found. Run /league/data/analytics/build-all first.")
        return data
    except HTTPException:
        raise
    except Exception as e:
